...
def gen_cloze_from_words(words, tags=None):
    tags_prompt = "; ".join(tags) if tags else "None"
    input_prompt = 'As an English learning assistant, your task is to create a cloze test that aids users in learning the input words. The input format is: [word1, word2, ...]. Each word should be utilized in the cloze test. Please also provide the answer of the cloze test.\nOutput format: {"content": cloze_test, "answer": [answer_word1, answer_word2, ...]}\nProvide your response in JSON format, with two keys: "content" and "answer". The value for "content" should include the cloze test, while the value for "answer" should be an list containing all the correct answers in sequence.\nRemember to incorporate context clues or hints within the text to assist users with unfamiliar vocabulary. Additionally, ensure your cloze test adheres to proper grammar and sentence structure.'
    if tags:
