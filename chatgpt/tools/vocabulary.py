...
def gen_cloze_from_words(words, tags=None):
    tags_prompt = "; ".join(tags) if tags else "None"
    input_prompt = 'As an English learning assistant, your task is to create a cloze test that aids users in learning the input words, and each used input word should be enclosed in "$" characters at both beginning and end. The input format is: [word1, word2, ...]. Each word should be utilized in the cloze test.\nOutput format: {"content": cloze_test}\nProvide your response in JSON format, with the key: "content". The value for "content" should include the cloze test.\nRemember to incorporate context clues or hints within the text to assist users with unfamiliar vocabulary. Additionally, ensure your cloze test adheres to proper grammar and sentence structure.'
    if tags:
